...
function updateGridState(newState) {
  if (PARAMETERIZE_GRID_STATE) {
    isGrid = newState;
    if (isGrid) {
      thePlayers.forEach((player) => {
        changeAbr(player, 0, 540);
      });
    } else {
      const bigScreen = getBigScreen();
      resetAbr(bigScreen); // allow automatic ABR
      const smallScreens = getSmallScreens();
      for (let i = 0; i < smallScreens.length; i++) {
        changeAbr(smallScreens[i], 0, 480);
      }
    }
  }
}
...
// the primary is the master angle
function getPrimary() {
  return THEOplayer.players.find((p) => p.uid == 0);
}

// the secondaries are the slave angles
function getSecondaries() {
  return THEOplayer.players.filter((p) => p.uid > 0);
}

function getBigScreen() {
  return THEOplayer.players.find(
    (p) =>
      p.element.parentNode.parentNode.parentNode.parentNode.id != "secondary"
  );
}
// let secondary = THEOplayer.players.filter((p) => p.muted == true);

function getSmallScreens() {
  return THEOplayer.players.filter(
    (p) =>
      p.element.parentNode.parentNode.parentNode.parentNode.id == "secondary"
  );
}
...
function createSecondaryPlayers(number) {
  ...
  multiCamCloseSection.innerHTML =
    '<div class="multiClose" onclick="closeListView();">X</div><h4>More camera angles</h4>';
...
function start404SegmentsForIndex(index) {
  console.log(
    "Start redirecting segments of stream for player",
    index,
    "to 404s..."
  );
...
function bufferCatchups() {
  let primary = getPrimary();
  let secondaries = getSecondaries();
  for (let i = 0; i < secondaries.length; i++) {
    bufferCatchup(secondaries[i], primary);
  }
}
function bufferCatchup(player, primary) {
  player.addEventListener("playing", function () {
    let diff = getDrift(primary, player);
    if (Math.abs(diff) > 2) {
      console.warn(
        "Doing buffer catch-up. Difference between two players exceeds 2 seconds. Forcing a sync between them. (",
        diff,
        "s.)"
      );
      forceSyncBetweenPlayers(player, primary);
    }
  });
}

// allows you to restrict the quality ladder of a player by specifying a min and max height resolution (e.g. changeAbr(player2, 540, 1080))
